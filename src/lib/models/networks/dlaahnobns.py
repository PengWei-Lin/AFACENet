...
class Conv_Relu_S(nn.Module):
    def __init__(self, in_size):
        super(Conv_Relu_S, self).__init__()
...
def dlaahnobns34(pretrained=True, **kwargs):  # DLA-34
...
class DLASeg(nn.Module):
    def __init__(self, base_name, heads, pretrained, down_ratio, final_kernel,
        ...
        #self.attention_head = Conv_Relu_S(in_size=3)
        self.attention_head = CSAModuleV2(in_size=3)
        self.wh_head = Conv_Relu(in_size=2)
        self.dim_head = Conv_Relu(in_size=3)
        self.dep_head = Conv_Relu(in_size=1)
        self.reg_head = Conv_Relu_S(in_size=2)
        self.act_head = Conv_PRelu(in_size=2)
        
        self.rot_head = Conv_PRelu(in_size=8)
...
def get_pose_net(num_layers, heads, head_conv=256, down_ratio=4):
    model = DLASeg('dlaahnobns{}'.format(num_layers), heads,
                   pretrained=True,
