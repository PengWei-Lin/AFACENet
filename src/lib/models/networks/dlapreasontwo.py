...
def dlapreasontwo34(pretrained=False, **kwargs):  # DLA-34
...
class DLASeg(nn.Module):
    def __init__(self, base_name, heads, pretrained, down_ratio, final_kernel,
        ...
        #self.hm_c = nn.Conv2d(3, 3, kernel_size=1, stride=1, bias=False)
        #fill_fc_weights(self.hm_c)
        '''
        ...
        for fpn_idx, fpn_c in enumerate(fpn_channels):
            for head in sorted(self.heads):
                ...
                if head_conv > 0:
                    if 'hm' in head:
                        fc = nn.Sequential(
                            nn.Conv2d(fpn_c, fpn_c, kernel_size=3, padding=1, bias=True),
                            nn.ReLU(inplace=True),
                            nn.Conv2d(fpn_c, classes,
...
def get_pose_net(num_layers, heads, head_conv=256, down_ratio=4):
    model = DLASeg('dlapreasontwo{}'.format(num_layers), heads,
                   pretrained=True,
