...
'''
class Bin1_activation(nn.Module):
    def __init__(self, beta):
        super(Bin1_activation, self).__init__()
        self.beta = beta
        
    def forward(self, x):
        x[x<=0]=1#0  #0
        x[x>0]=0#-10  #-5
        #x = 1/self.beta * torch.log(1 + torch.exp(self.beta * x))
...
class Bin2_activation(nn.Module):
    def __init__(self, beta):
        super(Bin2_activation, self).__init__()
        self.beta = beta
...
def dlaoffsetaware34(pretrained=True, **kwargs):  # DLA-34
...
class DLASeg(nn.Module):
    def __init__(self, base_name, heads, pretrained, down_ratio, final_kernel,
        ...
        '''
        self.act_bin1 = Bin1_activation(beta=1)
        self.act_bin2 = Bin2_activation(beta=1)
        
        self.lrp_bin1 = Learnable_Parameter(2, 1)
        self.lrp_bin2 = Learnable_Parameter(2, 1)
        '''
        self.heads = heads
    ...
    def forward(self, x):
        ...
        for head in self.heads:
            ...
            if head == 'hm':
                ...
                '''
                p_hm = torch.clone(final_out[:,0:1,:,:])
                p_hm[p_hm >= 0.5] = 1
                p_hm[p_hm < 1] = 0
                #pedestrian_hm = torch.clone(final_out[:,0:1,:,:])
                c_hm = torch.clone(final_out[:,1:2,:,:])
                c_hm[p_hm >= 0.5] = 1
                c_hm[p_hm < 1] = 0
                
                cy_hm = torch.clone(final_out[:,2:,:,:])
                cy_hm[p_hm >= 0.5] = 1
                cy_hm[p_hm < 1] = 0
                #tmp_hm = final_out
                '''
            '''
                ...
                act_y = final_out[:,0:1,:,:]
                #print("act_y:{}".format(act_y.shape))
            elif head == 'rot':
                bin1_off = self.act_bin1(act_y)
                bin2_off = self.act_bin2(act_y)
                
                #print("bin1_off:{}".format(bin1_off.shape))
                #print("bin2_off:{}".format(bin2_off.shape))
                
                #print("bin2_off:{}".format(bin2_off))
                
                rot_cls1 = final_out[:, 1:2, :, :]
                rot_cls2 = final_out[:, 5:6, :, :]
                
                tmp_rot1 = rot_cls1
                tmp_rot2 = rot_cls2
                
                #print("rot_cls1:{}".format(rot_cls1.shape))
                #print("rot_cls2:{}".format(rot_cls2.shape))
                
                lr_feature1 = torch.cat((bin1_off, rot_cls1), dim=1)
                lr_feature2 = torch.cat((bin2_off, rot_cls2), dim=1)
                
                #print("lr_feature1:{}".format(lr_feature1.shape))
                #print("lr_feature2:{}".format(lr_feature2.shape))
                
                
                alpha1 = self.lrp_bin1(lr_feature1)
                alpha2 = self.lrp_bin2(lr_feature2)
                
                
                #print("alpha1:{}".format(alpha1.shape))
                #print("alpha2:{}".format(alpha2.shape))
                ...
                rot_p0 = final_out[:,:1,:,:]
                #print("part0:{}".format(rot_p0.shape))
                
                rot_p1 = final_out[:,2:5,:,:]
                #print("part1:{}".format(rot_p1.shape))
                
                rot_p2 = final_out[:,6:8,:,:]
                #print("part2:{}".format(rot_p2.shape))
                
                rot_cls1 = (alpha1 * bin1_off) + ((1 - alpha1) * rot_cls1)
                rot_cls2 = (alpha2 * bin2_off) + ((1 - alpha2) * rot_cls2)
                
                #rot_cls1 = rot_cls1 * tmp_hm[:,0:1,:,:]
                #rot_cls2 = rot_cls2 * tmp_hm[:,0:1,:,:]
                
                rot_cls1 = rot_cls1 * p_hm
                rot_cls2 = rot_cls2 * p_hm
                ...
                tmp_rot1 = (cc_hm) * tmp_rot1
                tmp_rot2 = (cc_hm) * tmp_rot2
                
                
                #tmp_rot1 = (c_hm + cy_hm) * tmp_rot1
                #tmp_rot2 = (c_hm + cy_hm) * tmp_rot2
                
                rot_cls1 = rot_cls1 + tmp_rot1
                rot_cls2 = rot_cls2 + tmp_rot2
                
                #print("rot_cls1:{}".format(rot_cls1.shape))
                #print("rot_cls2:{}".format(rot_cls2.shape))
                
                rot_final = torch.cat((rot_p0, rot_cls1), dim=1)
                #print("rot_final:{}".format(rot_final.shape))
                
                rot_final = torch.cat((rot_final, rot_p1), dim=1)
                #print("rot_final:{}".format(rot_final.shape))
                
                rot_final = torch.cat((rot_final, rot_cls2), dim=1)
                #print("rot_final:{}".format(rot_final.shape))
                
                rot_final = torch.cat((rot_final, rot_p2), dim=1)
                #print("rot_final:{}".format(rot_final.shape))
...
def get_pose_net(num_layers, heads, head_conv=256, down_ratio=4):
    model = DLASeg('dlaoffsetaware{}'.format(num_layers), heads,
                   pretrained=True,
