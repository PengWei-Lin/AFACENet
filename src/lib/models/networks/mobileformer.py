...
class MobileFormer(nn.Module):
    ...
    def forward(self, x):
        ...
        for block in self.blocks:
            ...
            xb, xc, xh, xw = x.size()
            tn, tm, td = tokens.size()
            #tt = tokens
            #torch.reshape(tt, (xh, xw))
            print("CNN feature N:{}, C:{}, H:{}, W:{}".format(xb, xc, xh, xw))
            print("Transformer feature N:{}, M:{}, D{}".format(tn, tm, td))
            tt = tokens
            tt = torch.reshape(tt, (-1,))
            ttd = tt.size()
            print("Reshape {}".format(ttd))
        x = self.project(x)
        xb, xc, xh, xw = x.size()
        print("CNN feature After porjrct N:{}, C:{}, H:{}, W:{}".format(xb, xc, xh, xw))
        x = self.avgpool(x).squeeze()
        xb, xc = x.size()
        print("CNN feature After Pooling N:{}, C:{}".format(xb, xc))
        print("token length:{}!!!!!!!!!!!!!!!!!!!!".format(len(tokens[:, 0, :])))
        tn, tm, td = tokens.size()
        print("Token feature N:{}, M:{}, D{}".format(tn, tm, td))
        ts1, ts2 = tokens[:, 0 , :].size()
        print("before cat {}, {}".format(ts1, ts2))
        x = torch.cat([x, tokens[:, 0, :]], dim=-1)
        n, m = x.size()
        print("Feature with Tokens  N:{}, M:{}".format(n, m))
        x = self.fc(x)
        n, m = x.size()
        print("Feature with Tokens after FC  N:{}, M:{}".format(n, m))
...
if __name__ == '__main__':
    ...
    summary(mobile_former_96(1000), (3, 384, 1280))