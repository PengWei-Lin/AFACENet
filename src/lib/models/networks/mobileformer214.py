...
class Mobile2Former(nn.Module):
    ...
    def forward(self, x, z):
        b, m, d = z.shape
        print("Z b:{}, m:{}, d:{}".format(b,m,d))
        b, c, h, w = x.shape
        print("X b:{}, c:{}, h:{}, w{}".format(b,c,h,w))
...
'Unoffical'
class MobileFormer214(nn.Module):
    def __init__(self, heads, final_kernel, head_conv):
        super(MobileFormer214, self).__init__()
        ...
        cfg = {
            ...
            'body': [
                # stage2
                {'inp': 12, 'exp': 72, 'out': 20, 'se': None, 'stride': 2, 'heads': 2, 'dim': 192},
                {'inp': 20, 'exp': 60, 'out': 20, 'se': None, 'stride': 1, 'heads': 2, 'dim': 192},
                # stage3
                {'inp': 20, 'exp': 120, 'out': 40, 'se': None, 'stride': 2, 'heads': 2, 'dim': 192},
                {'inp': 40, 'exp': 160, 'out': 40, 'se': None, 'stride': 1, 'heads': 2, 'dim': 192},
                # stage4
                {'inp': 40, 'exp': 240, 'out': 80, 'se': None, 'stride': 2, 'heads': 2, 'dim': 192},
                {'inp': 80, 'exp': 320, 'out': 80, 'se': None, 'stride': 1, 'heads': 2, 'dim': 192},
                {'inp': 80, 'exp': 480, 'out': 112, 'se': None, 'stride': 1, 'heads': 2, 'dim': 192},
                {'inp': 112, 'exp': 672, 'out': 112, 'se': None, 'stride': 1, 'heads': 2, 'dim': 192},
                # stage5
                {'inp': 112, 'exp': 672, 'out': 160, 'se': None, 'stride': 2, 'heads': 2, 'dim': 192},
                {'inp': 160, 'exp': 960, 'out': 160, 'se': None, 'stride': 1, 'heads': 2, 'dim': 192},
                {'inp': 160, 'exp': 960, 'out': 160, 'se': None, 'stride': 1, 'heads': 2, 'dim': 192},
            ],
            'fc1': 1024,  # hid_layer
        ...
        self.block0 = nn.Sequential(
            BaseBlock(inp=12, exp=72, out=20, se=None, stride=2, heads=2, dim=192),
            BaseBlock(inp=16, exp=60, out=20, se=None, stride=1, heads=2, dim=192),
        )
        self.block1 = nn.Sequential(
            BaseBlock(inp=20, exp=120, out=40, se=None, stride=2, heads=2, dim=192),
            BaseBlock(inp=40, exp=120, out=40, se=None, stride=1, heads=2, dim=192),
        )
        self.block2 = nn.Sequential(
            BaseBlock(inp=40, exp=240, out=80, se=None, stride=2, heads=2, dim=192),
            BaseBlock(inp=80, exp=320, out=80, se=None, stride=1, heads=2, dim=192),
            BaseBlock(inp=80, exp=480, out=112, se=None, stride=1, heads=2, dim=192),
            BaseBlock(inp=112, exp=672, out=112, se=None, stride=1, heads=2, dim=192),
        )
        self.block3 = nn.Sequential(
            BaseBlock(inp=112, exp=672, out=160, se=None, stride=2, heads=2, dim=192),
            BaseBlock(inp=160, exp=960, out=160, se=None, stride=1, heads=2, dim=192),
            BaseBlock(inp=160, exp=960, out=160, se=None, stride=1, heads=2, dim=192),
        )
        
        #inp = 96
        #exp = 576
        self.conv = nn.Conv2d(160, 960, kernel_size=1, stride=1, padding=0, bias=False)
        ...
        self.ida_up = IDAUp(20, [20, 40, 112, 160], [2 ** i for i in range(4)])
        ...
        for head_id, head in enumerate(self.heads):
            ...
            if head_id == 0:
                fc = nn.Sequential(
                    nn.Conv2d(20, head_conv, kernel_size=3, padding=1, bias=True),
                    nn.ReLU(inplace=True),
