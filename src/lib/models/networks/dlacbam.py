...
class CBAM(nn.Module):
    def __init__(self, in_size):
        ...
        self.fc_ch = nn.Sequential(nn.Conv2d(in_size, in_size // 1, 1, bias=False),
                               nn.ReLU(),
                               nn.Conv2d(in_size // 1, in_size, 1, bias=False),
                               )
...
def dlacbam34(pretrained=True, **kwargs):  # DLA-34
...
def get_pose_net(num_layers, heads, head_conv=256, down_ratio=4):
    model = DLASeg('dlacbam{}'.format(num_layers), heads,
                   pretrained=True,
