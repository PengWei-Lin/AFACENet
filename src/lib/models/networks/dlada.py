...
class DAModule(nn.Module):
    ...
    def __init__(self,d_model=3,kernel_size=3,H=7,W=7):
        super().__init__()
        self.position_attention_module=PositionAttentionModule(d_model=3,kernel_size=3,H=7,W=7)
        self.channel_attention_module=ChannelAttentionModule(d_model=3,kernel_size=3,H=7,W=7)
...
def dlada34(pretrained=True, **kwargs):  # DLA-34
...
def get_pose_net(num_layers, heads, head_conv=256, down_ratio=4):
    model = DLASeg('dlada{}'.format(num_layers), heads,
                   pretrained=True,
