...
class MobileViTxs(nn.Module):
    def __init__(self, heads, final_kernel, head_conv):
        super(MobileViTxs, self).__init__()
        ih = 384
        iw = 1280
        ph = 2
        pw = 2
        ...
        self.first_mv2 = MV2Block(16, 32, 1, 4)
        ...
        self.block1 = nn.Sequential(
            MV2Block(48, 64, 2, 4),
            MobileViTBlock(96, 2, 64, 3, (2, 2), 192),
        )
        
        self.block2 = nn.Sequential(
            MV2Block(64, 80, 2, 4),
            MobileViTBlock(120, 4, 80, 3, (2, 2), 480)
        )
        
        self.block3 = nn.Sequential(
            MV2Block(80, 96, 2, 4),
            MobileViTBlock(144, 3, 96, 3, (2, 2), 576),
        )
        '''
        ...
        self.conv2 = conv_1x1_bn(96, 384)
        #self.pool = nn.AvgPool2d(ih//32, 1)
        #self.fc = nn.Linear(channels[-1], num_classes, bias=False)
        self.init_params()

        self.ida_up = IDAUp(48, [48, 64, 80, 384], [2 ** i for i in range(4)])
        ...
        for head_id, head in enumerate(self.heads):
            ...
            if head_id == 0:
                fc = nn.Sequential(
                    nn.Conv2d(48, head_conv, kernel_size=3, padding=1, bias=True),
                    nn.ReLU(inplace=True),
