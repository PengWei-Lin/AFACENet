...
class Inverted_Bottleneck(nn.Module):
    def __init__(self, inplanes, planes, stride=1, dilation=1):
        ...
        expansion = 4
        hidden = expansion * planes
        self.conv1 = nn.Conv2d(inplanes, hidden, kernel_size=1, stride=1, padding=0, bias=False, dilation=dilation, groups=1)
        self.bn1 = nn.BatchNorm2d(hidden, momentum=BN_MOMENTUM)
        
        self.conv2 = nn.Conv2d(hidden, hidden, kernel_size=3, stride=stride, padding=1, bias=False, dilation=dilation, groups=hidden)
        self.bn2 = nn.BatchNorm2d(hidden, momentum=BN_MOMENTUM)
        
        self.conv3 = nn.Conv2d(hidden, planes, kernel_size=1, stride=1, padding=0, bias=False, dilation=dilation, groups=1)
...
def ldla34(pretrained=False, **kwargs):  # DLA-34
    model = DLA([1, 1, 1, 2, 2, 1],
                #[16, 32, 64, 128, 256, 512],
                [8, 16, 32, 64, 128, 256],
                #block=BasicBlock, **kwargs)
                block=Inverted_Bottleneck, **kwargs)
    #if pretrained:
...
def get_pose_net(num_layers, heads, head_conv=64, down_ratio=4):
  model = DLASeg('ldla{}'.format(num_layers), heads,
                 pretrained=True,
