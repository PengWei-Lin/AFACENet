...
'Unoffical'
class MobileFormer96(nn.Module):
    def __init__(self, heads, final_kernel, head_conv):
        super(MobileFormer96, self).__init__()
        self.token = nn.Parameter(nn.Parameter(torch.randn(1, 4, 128)))
        ...
        cfg = {
            ...
            'body': [
                # stage2
                {'inp': 12, 'exp': 72, 'out': 16, 'se': None, 'stride': 2, 'heads': 2, 'dim': 128},
                # stage3
                {'inp': 16, 'exp': 96, 'out': 32, 'se': None, 'stride': 2, 'heads': 2, 'dim': 128},
                {'inp': 32, 'exp': 96, 'out': 32, 'se': None, 'stride': 1, 'heads': 2, 'dim': 128},
                # stage4
                {'inp': 32, 'exp': 192, 'out': 64, 'se': None, 'stride': 2, 'heads': 2, 'dim': 128},
                {'inp': 64, 'exp': 256, 'out': 64, 'se': None, 'stride': 1, 'heads': 2, 'dim': 128},
                {'inp': 64, 'exp': 384, 'out': 88, 'se': None, 'stride': 1, 'heads': 2, 'dim': 128},
                # stage5
                {'inp': 88, 'exp': 528, 'out': 128, 'se': None, 'stride': 2, 'heads': 2, 'dim': 128},
                {'inp': 128, 'exp': 768, 'out': 128, 'se': None, 'stride': 1, 'heads': 2, 'dim': 128},
            ],
            'fc1': 1024,  # hid_layer
        ...
        self.block0 = nn.Sequential(
            BaseBlock(inp=12, exp=72, out=16, se=None, stride=2, heads=2, dim=128),
        )
        self.block1 = nn.Sequential(
            BaseBlock(inp=16, exp=96, out=32, se=None, stride=2, heads=2, dim=128),
            BaseBlock(inp=32, exp=96, out=32, se=None, stride=1, heads=2, dim=128),
        )
        self.block2 = nn.Sequential(
            BaseBlock(inp=32, exp=192, out=64, se=None, stride=2, heads=2, dim=128),
            BaseBlock(inp=64, exp=256, out=64, se=None, stride=1, heads=2, dim=128),
            BaseBlock(inp=64, exp=384, out=88, se=None, stride=1, heads=2, dim=128),
        )
        self.block3 = nn.Sequential(
            BaseBlock(inp=88, exp=528, out=128, se=None, stride=2, heads=2, dim=128),
            BaseBlock(inp=128, exp=768, out=128, se=None, stride=1, heads=2, dim=128),
        )
