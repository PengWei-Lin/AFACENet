        ...
        for i, c in enumerate(channels):
            ...
            if f == 1:
                ...
                up = nn.Sequential(
                        ...
                        nn.Conv2d(out_dim, out_dim, kernel_size=1, stride=1, padding=0, groups=out_dim, bias=False)
                )
...
class DLASeg(nn.Module):
    def __init__(self, base_name, heads,
        ...
        '''
        self.DepthWise1 = nn.Conv2d(3, 3, kernel_size=1, stride=1, padding=0, bias=True, groups=3)
        self.DepthWise2 = nn.Conv2d(3, 3, kernel_size=1, stride=1, padding=0, bias=True, groups=3)
            ...
            if head_conv > 0:
                fout = nn.Sequential(
                  ...
                  nn.Conv2d(head_conv, classes, 
                    ...
                    padding=0, bias=True))
                if 'hm' in head:
                    fout[-1].bias.data.fill_(-2.19)
                else:
                    fill_fc_weights(fout)
            else:
                print("Count!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
                fc = nn.Conv2d(channels[self.first_level], classes, 
                  kernel_size=1, stride=1, 
                  padding=0, bias=True)
                at1 = self.DepthWise1(fc)
                at2 = self.DepthWise2(at1)
                at2_re = self.relu_at(at2)
                fout = at1 * at2_re
                if 'hm' in head:
                    fc.bias.data.fill_(-2.19)
                else:
                    fill_fc_weights(fc)
            self.__setattr__(head, fout)
