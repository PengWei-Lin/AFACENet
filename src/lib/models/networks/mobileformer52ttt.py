...
class MobileUp(nn.Module):
    def __init__(self, f, inp, hid, out, se, dim, reduction=4, k=2):
        super(MobileUp, self).__init__()
        self.dim = dim
        self.hid, self.out = hid, out
        self.k = k
        self.fc1 = nn.Linear(dim, dim // reduction)
        ...
        self.se = se
        ######################################
        self.dw_conv1 = nn.ConvTranspose2d(inp, hid, kernel_size=f*2, stride=f, padding=f // 2, groups=inp, bias=False)
        self.dw_bn1 = nn.BatchNorm2d(hid)
        self.dw_act1 = MyDyRelu(2)
        ...
        self.dw_conv2 = nn.Conv2d(inp, hid, kernel_size=3, stride=1, padding=3 // 2, groups=inp, bias=False)
        self.dw_bn2 = nn.BatchNorm2d(hid)
        self.dw_act2 = MyDyRelu(2)
...
class UpBlock(nn.Module):
    def __init__(self, f, inp, exp, out, se, heads, dim):
        super(UpBlock, self).__init__()
        self.mobile = MobileUp(f, inp, exp, out, se, dim)
        self.mobile2former = Mobile2Former(dim=dim, heads=heads, channel=inp)
        self.former = Former(dim=dim)
        self.former2mobile = Former2Mobile(dim=dim, heads=heads, channel=out)
...
'Unoffical'
class MobileFormer52ttt(nn.Module):
    def __init__(self, heads, final_kernel, head_conv):
        super(MobileFormer52ttt, self).__init__()
        ...
        # L1
        self.up_blk3_16s = nn.Sequential(
            UpBlock(f=2, inp=96, exp=192, out=24, se=None, heads=2, dim=128),
        )
        
        self.up_blk2_8s = nn.Sequential(
            UpBlock(f=2, inp=64, exp=128, out=16, se=None, heads=2, dim=128),
        )
        
        self.up_blk1_4s = nn.Sequential(
            UpBlock(f=2, inp=24, exp=48, out=6, se=None, heads=2, dim=128),
        )
        
        
        
        # L2
        self.up_x2x3_8s = nn.Sequential(
            UpBlock(f=2, inp=22, exp=44, out=22, se=None, heads=2, dim=128),
        )
        
        
        self.up_x1x2_4s = nn.Sequential(
            UpBlock(f=2, inp=10, exp=30, out=10, se=None, heads=2, dim=128),
        )
        
        
        
        # L3
        self.up_x1x2x3_4s = nn.Sequential(
            UpBlock(f=2, inp=32, exp=64, out=32, se=None, heads=2, dim=128),
        )
        
        
        
        #DUP 16->4
        self.d16up4s = nn.Sequential(
            UpBlock(f=4, inp=22, exp=66, out=22, se=None, heads=2, dim=128),
        )
        
        
        
        # L1
        self.c16s = nn.Sequential(
            BaseBlock(inp=88, exp=176, out=22, se=None, stride=1, heads=2, dim=128),
        )
        self.c8s = nn.Sequential(
            BaseBlock(inp=40, exp=80, out=10, se=None, stride=1, heads=2, dim=128),
        )
        self.c4s = nn.Sequential(
            BaseBlock(inp=18, exp=36, out=6, se=None, stride=1, heads=2, dim=128),
        )
        
        
        # L2
        self.l2c8s = nn.Sequential(
            BaseBlock(inp=32, exp=64, out=32, se=None, stride=1, heads=2, dim=128),
        )
        self.l2c4s = nn.Sequential(
            BaseBlock(inp=16, exp=48, out=16, se=None, stride=1, heads=2, dim=128),
        )
        
        
        # L3
        self.l3c4s = nn.Sequential(
            BaseBlock(inp=48, exp=96, out=48, se=None, stride=1, heads=2, dim=128),
        )
        ...
        # Out
        self.s32out = nn.Sequential(
            BaseBlock(inp=96, exp=576, out=512, se=None, stride=1, heads=2, dim=128),
        )
        
        
        self.s16out = nn.Sequential(
            BaseBlock(inp=22, exp=220, out=256, se=None, stride=1, heads=2, dim=128),
        )
        
        
        self.s8out = nn.Sequential(
            BaseBlock(inp=32, exp=128, out=128, se=None, stride=1, heads=2, dim=128),
        )
        
        
        self.s4out = nn.Sequential(
            BaseBlock(inp=102, exp=408, out=64, se=None, stride=1, heads=2, dim=128),
        )
        
        #inp = 96
        #exp = 576 ########## with 1x1 convolution and bn big start loss!!!!
        #self.conv = nn.Conv2d(96, 768, kernel_size=1, stride=1, padding=0, bias=False)
    ...
    def forward(self, x):
        ...
        x0, z0 = self.block0([x, z])       #12
        x1, z1 = self.block1([x0, z0])     #24
        x2, z2 = self.block2([x1, z1])     #64
        x3, z3 = self.block3([x2, z2])     #96
        
        
        s32o, s32oz = self.s32out([x3, z3])
        #x3_f = self.bn(self.conv(x3))
        #b, c, h, w = x3_f.size()
        #print("x3_f: {}, {}, {}, {}".format(b,c,h,w))
        
        
        # L1
        x3_16s, z3_16s = self.up_blk3_16s([x3, z3])  #24
        x2_x3, x2_x3z = self.c16s([torch.cat((x2 ,x3_16s), dim=1), z3_16s])  #22          16S
        s16o, s16oz = self.s16out([x2_x3, x2_x3z]) # L1  out
        
        x2_8s, z2_8s = self.up_blk2_8s([x2, z2])  #16
        x2_x1, x2_x1z = self.c8s([torch.cat((x1, x2_8s), dim=1), z2_8s])  #10            8s
        
        x1_4s, z1_4s = self.up_blk1_4s([x1, z1])  #6
        x0_x1, x0_x1z = self.c4s([torch.cat((x0, x1_4s), dim=1), z1_4s])  #6            4s
        
        
        # L2
        x2x3u, x2x3uz = self.up_x2x3_8s([x2_x3, x2_x3z]) #       22                          8s
        x1x2x3_8s, x1x2x3_8sz = self.l2c8s([torch.cat((x2_x1, x2x3u), dim=1), x2x3uz]) #    32          8s
        s8o, s8oz = self.s8out([x1x2x3_8s, x1x2x3_8sz])# L2 out
        
        x1x2u, x1x2uz = self.up_x1x2_4s([x2_x1, x2_x1z])  #  10                             4s
        x0x1x2_4s, x0x1x2_4sz = self.l2c4s([torch.cat((x0_x1, x1x2u), dim=1), x1x2uz])  #   16      4s
        
        
        # L3
        x1x2x3u, x1x2x3uz = self.up_x1x2x3_4s([x1x2x3_8s, x1x2x3_8sz]) #         32                4s
        x0x1x2x3, x0x1x2x3z = self.l3c4s([torch.cat((x0x1x2_4s, x1x2x3u), dim=1), x1x2x3uz]) #   48       4s
        
        
        ########dup
        x2x3du, x2x3duz = self.d16up4s([x2_x3, x2_x3z])     #  22        4s
        
        ######concate
        a3 = torch.cat((x1x2x3u, x0x1x2x3, x2x3du), dim=1)     # 22+48+32
        s4o, s4oz = self.s4out([a3, x2x3duz])
        ...
        #outx = [x0, x1, x2, x3]
        #outz = [z0, z1, z2, z3]
        #outx = [x3_4scf, x3_8sc, x3_16scf, x3_f]
        #outz = [z0, z1, z2, z3]
        
        outx = [s4o, s8o, s16o, s32o]
