...
'Unoffical'
class MobileFormer508(nn.Module):
    def __init__(self, heads, final_kernel, head_conv):
        super(MobileFormer508, self).__init__()
        ...
        self.bneck = nn.Sequential(
            nn.Conv2d(24, 48, 3, stride=1, padding=1, groups=24),
            hswish(),
        ...
        self.block0 = nn.Sequential(
            BaseBlock(inp=24, exp=144, out=40, se=None, stride=2, heads=2, dim=192),
            BaseBlock(inp=40, exp=120, out=40, se=None, stride=1, heads=2, dim=192),
        )
        self.block1 = nn.Sequential(
            BaseBlock(inp=40, exp=240, out=72, se=None, stride=2, heads=2, dim=192),
            BaseBlock(inp=72, exp=216, out=72, se=None, stride=1, heads=2, dim=192),
        )
        self.block2 = nn.Sequential(
            BaseBlock(inp=72, exp=432, out=128, se=None, stride=2, heads=2, dim=192),
            BaseBlock(inp=128, exp=384, out=128, se=None, stride=1, heads=2, dim=192),
            BaseBlock(inp=128, exp=768, out=176, se=None, stride=1, heads=2, dim=192),
            BaseBlock(inp=176, exp=1056, out=176, se=None, stride=1, heads=2, dim=192),
        )
        self.block3 = nn.Sequential(
            BaseBlock(inp=176, exp=1056, out=240, se=None, stride=2, heads=2, dim=192),
            BaseBlock(inp=240, exp=1440, out=240, se=None, stride=1, heads=2, dim=192),
            BaseBlock(inp=240, exp=1440, out=240, se=None, stride=1, heads=2, dim=192),
        )
        
        #inp = 96
        #exp = 576
        self.conv = nn.Conv2d(240, 1440, kernel_size=1, stride=1, padding=0, bias=False)
        self.bn = nn.BatchNorm2d(1440)
        ...
        self.ida_up = IDAUp(40, [40, 72, 176, 1440], [2 ** i for i in range(4)])
        ...
        for head_id, head in enumerate(self.heads):
            ...
            if head_id == 0:
                fc = nn.Sequential(
                    nn.Conv2d(40, head_conv, kernel_size=3, padding=1, bias=True),
                    nn.ReLU(inplace=True),
