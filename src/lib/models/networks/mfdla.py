...
class Tree(nn.Module):
    def __init__(self, levels, block, in_channels, out_channels, stride=1,
        ...
        if levels == 1:
            ...
            self.tree1 = block(inp=in_channels, exp=2*in_channels, out=out_channels, se=None, stride=stride, heads=2, dim=128)
            #self.tree1 = block(in_channels, out_channels, 1, dilation=dilation)
            self.tree1 = block(inp=in_channels, exp=2*in_channels, out=out_channels, se=None, stride=1, heads=2, dim=128)
        else:
    ...
    def forward(self, x, residual=None, children=None):
        ...
        bottom = x#self.downsample(x) if self.downsample else x
...
class MTDLA(nn.Module):
    def __init__(self, levels, channels, num_classes=1000,
                 block=BaseBlock, residual_root=False, linear_root=False):
        super(MTDLA, self).__init__()
        ...
        self.level3 = Tree(levels[3], block, channels[2], channels[3], 2,
                           level_root=False, root_residual=residual_root) # T
    ...
    def forward(self, x):
        ...
        x = self.level1(x)
        y.append(x)
        x2, z2 = self.level2([x, z])
        y.append(x2)
        x3, z3 = self.level3([x2, z2])
        y.append(x3)
        x4, z4 = self.level4([x3, z3])
        y.append(x4)
        x5, z5 = self.level5([x4, z4])
        y.append(x5)
...
def mfdla34(pretrained=False, **kwargs):  # DLA-34
    model = MTDLA([1, 1, 1, 2, 2, 1],
                [16, 32, 64, 128, 256, 512],
                block=BaseBlock, **kwargs)
...
def get_pose_net(num_layers, heads, head_conv=256, down_ratio=4):
  model = DLASeg('mfdla{}'.format(num_layers), heads,
                 pretrained=False,
