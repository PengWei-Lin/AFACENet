...
def dlaahpcsa34(pretrained=True, **kwargs):  # DLA-34
...
class DLASeg(nn.Module):
    def __init__(self, base_name, heads, pretrained, down_ratio, final_kernel,
        ...
        self.wh_head = Conv_BN_PRelu(in_size=2)
        self.dim_head = Conv_BN_PRelu(in_size=3)
        self.dep_head = Conv_BN_PRelu(in_size=1)
        self.reg_head = Conv_BN_PRelu(in_size=2)
...
def get_pose_net(num_layers, heads, head_conv=256, down_ratio=4):
    model = DLASeg('dlaahpcsa{}'.format(num_layers), heads,
                   pretrained=True,
