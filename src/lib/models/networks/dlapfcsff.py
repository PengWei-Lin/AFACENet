...
class SFF(nn.Module):
    def __init__(self, in_size):
        ...
        self.sacf = nn.Sequential(
            nn.Conv2d(in_size, in_size, kernel_size=3, stride=1, padding=1, bias=True, groups=in_size),  ##### depthwise....
        )
            
        self.cbr = nn.Sequential(
            nn.Conv2d(in_size*2, in_size, kernel_size=3, stride=1, padding=1, bias=True, groups=in_size),  ##### depthwise....
...
def dlapfcsff34(pretrained=True, **kwargs):  # DLA-34
...
def get_pose_net(num_layers, heads, head_conv=256, down_ratio=4):
    model = DLASeg('dlapfcsff{}'.format(num_layers), heads,
                   pretrained=True,
