...
class SCConv(nn.Module):
    def __init__(self, in_channels, out_channels, stride, pooling_r):
        ...
        self.conv1 = nn.Conv2d(in_channels, in_channels//2, kernel_size=1, stride=1, bias=False)
        
        
        self.conv2 = nn.Conv2d(in_channels, in_channels//2, kernel_size=1, stride=1, bias=False)
        
        
        self.k1 = nn.Sequential(
            nn.Conv2d(in_channels//2, out_channels//2, kernel_size=3, stride=stride, padding=1, bias=False),
        ...
        self.identity_conv = nn.Conv2d(in_channels//2, out_channels//2, kernel_size=1, stride=1, padding=0, bias=False)
    ...
    def forward(self, x):
        ...
        if self.in_channels != self.out_channels:
            identity = self.identity_conv(x1)
        else:
            identity = x1

        y1 = torch.sigmoid(torch.add(identity, F.interpolate(self.k2(x1), identity.size()[2:])))
        y1 = torch.mul(self.k3(x1), y1)
        y1 = self.k4(y1)
...
class SCConvFDC(nn.Module):
    def __init__(self, in_channels, out_channels, stride, pooling_r):
        super(SCConvFDC, self).__init__()
        ...
        self.k1 = nn.Sequential(
            nn.Conv2d(in_channels//2, out_channels, kernel_size=3, stride=stride, padding=1, bias=False),
        ...
        #if in_channels != out_channels:
        self.identity_conv = nn.Conv2d(in_channels//2, out_channels, kernel_size=1, stride=1, padding=0, bias=False)
        self.in_channels = in_channels
        self.out_channels = out_channels
...
class SCConvCA(nn.Module):
    def __init__(self, in_channels, out_channels, stride, pooling_r):
        super(SCConvCA, self).__init__()
        ...
        #self.k1[-2].weight.data.fill_(0)
        #self.k1[-2].bias.data.fill_(0)
        ...
        #self.k2[-2].weight.data.fill_(0)
        #self.k2[-2].bias.data.fill_(0)
        ...
        #self.k3[-2].weight.data.fill_(0)
        #self.k3[-2].bias.data.fill_(0)
        ...
        #self.k4[-3].weight.data.fill_(0)
        #self.k4[-3].bias.data.fill_(0)
        ...
        self.carafe = CARAFEPack(channels = out_channels//2, scale_factor = pooling_r, up_kernel = 5, up_group = 1, encoder_kernel = 3, encoder_dilation = 1, compressed_channels = 64)
        
        #if in_channels != out_channels:
        self.identity_conv = nn.Conv2d(in_channels//2, out_channels//2, kernel_size=1, stride=1, padding=0, bias=False)
        
        #nn.init.constant_(self.identity_conv.weight, 0.)
        #nn.init.constant_(self.identity_conv.bias, 0.)
    ...
    def forward(self, x):
        ...
        #y1 = torch.sigmoid(torch.add(identity, F.interpolate(self.k2(x1), identity.size()[2:])))
        y1 = torch.sigmoid(torch.add(identity, self.carafe(self.k2(x1))))
        y1 = torch.mul(self.k3(x1), y1)
        y1 = self.k4(y1)
...
class SCConvr(nn.Module):
    def __init__(self, in_channels, out_channels, stride, pooling_r):
        super(SCConvr, self).__init__()
...
class SCConvh(nn.Module):
    def __init__(self, in_channels, out_channels, stride, pooling_r):
        super(SCConvh, self).__init__()
...
class SCConvSP(nn.Module):
    def __init__(self, in_channels, pooling_r):
        super(SCConvSP, self).__init__()
        ...
        self.conv3 = nn.Conv2d(in_channels*2, in_channels, kernel_size=1, stride=1, bias=False)
    ...
    def forward(self, x):
        ...
        #y1 = torch.sigmoid(torch.add(identity, F.interpolate(self.k2(x1), identity.size()[2:])))
        y1 = torch.sigmoid(torch.add(x1, F.interpolate(self.k2(x1), x1.size()[2:])))
        y1 = torch.mul(self.k3(x1), y1)
        y1 = self.k4(y1)
...
class CSAModuleV3(nn.Module):
    def __init__(self, in_size):
        super(CSAModuleV3, self).__init__()
        self.ch_at = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),
            nn.Conv2d(in_size, in_size, kernel_size=1,
                      stride=1, padding=0, bias=True),
        ...
        self.sp_at = nn.Sequential(
            SCConvSP(in_channels=in_size, pooling_r=4),
            nn.Sigmoid(),
        )
...
class CSAModuleV4(nn.Module):
    def __init__(self, in_size):
        super(CSAModuleV4, self).__init__()
        self.ch_at = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),
            nn.Conv2d(in_size, in_size, kernel_size=1,
                      stride=1, padding=0, bias=True),
        ...
        self.sp_at1 = nn.Sequential(
            nn.Conv2d(in_size, in_size, kernel_size=1, stride=1, padding=0, bias=True, groups=1),
            nn.Sigmoid(),
        )
        
        self.sp_at2 = nn.Sequential(
            nn.Conv2d(in_size, in_size, kernel_size=3, stride=1, padding=1, bias=True, groups=1),
            nn.Sigmoid(),
        )
        
        self.sp_at3 = nn.Sequential(
            nn.Conv2d(in_size, in_size, kernel_size=5, stride=1, padding=2, bias=True, groups=1),
            nn.Sigmoid(),
        )
        
        self.sp_at4 = nn.Sequential(
            nn.Conv2d(in_size, in_size, kernel_size=7, stride=1, padding=3, bias=True, groups=1),
            nn.Sigmoid(),
        )
    ...
    def forward(self, x):
        ...
        return ch_out + sp_out1 + sp_out2 + sp_out3 + sp_out4
...
class CBAM(nn.Module):
    def __init__(self, in_size):
        ...
        self.fc_ch = nn.Sequential(nn.Conv2d(in_size, in_size // 16, 1, bias=False),
                               nn.ReLU(),
                               nn.Conv2d(in_size // 16, in_size, 1, bias=False),
                               )
        self.sigmoid_ch = nn.Sigmoid()
        
        self.conv_sp = nn.Conv2d(2, 1, kernel_size=7, stride=1, padding=3, bias=False)
        self.sigmoid_sp = nn.Sigmoid()
        
    def forward(self, x):
        ch_avg = self.avg_ch(x)
        ch_max = self.max_ch(x)
        
        ch_at = self.sigmoid_ch(self.fc_ch(ch_avg + ch_max)) * x
        
        sp_avg = torch.mean(ch_at, dim=1, keepdim=True)
        sp_max = torch.mean(ch_at, dim=1, keepdim=True)
        sp_cat = torch.cat([sp_avg, sp_max], dim=1)
        sp_at = self.sigmoid_sp(self.conv_sp(sp_cat)) * ch_at
...
class BasicBlock(nn.Module):
    ...
    def forward(self, x, residual=None):
        ...
        #print("before cbam: {}".format(out.shape))
        ...
        #print("after cbam: {}".format(out.shape))
...
class BasicSCBlock(nn.Module):
    def __init__(self, inplanes, planes, stride=1, dilation=1):
        super(BasicSCBlock, self).__init__()
        self.conv1 = SCConvr(inplanes, planes, stride=stride, pooling_r=4)
        self.bn1 = nn.BatchNorm2d(planes, momentum=BN_MOMENTUM)
        self.relu = nn.ReLU(inplace=True)
        self.conv2 = SCConvh(planes, planes, stride=1, pooling_r=4)
        self.bn2 = nn.BatchNorm2d(planes, momentum=BN_MOMENTUM)
        self.stride = stride
...
class BasicSChBlock(nn.Module):
    def __init__(self, inplanes, planes, stride=1, dilation=1):
        super(BasicSChBlock, self).__init__()
        self.conv1 = nn.Conv2d(inplanes, planes, kernel_size=3,
                               stride=stride, padding=dilation,
                               bias=False, dilation=dilation)
        self.bn1 = nn.BatchNorm2d(planes, momentum=BN_MOMENTUM)
...
def dlasc34(pretrained=True, **kwargs):  # DLA-34
    model = DLA([1, 1, 1, 2, 2, 1],
                [16, 32, 64, 128, 256, 512],
                block=BasicBlock, **kwargs)
    #block=BasicSChBlock, **kwargs)
    # block=BBlock, **kwargs)
...
class DeformConvSC(nn.Module):
    def __init__(self, chi, cho, pooling_r):
        super(DeformConvSC, self).__init__()
        self.actf = nn.Sequential(
            nn.BatchNorm2d(cho, momentum=BN_MOMENTUM),
            nn.ReLU(inplace=True)
        )
        
        self.offset_conv = SCConv(chi, 18, stride=1, pooling_r=pooling_r)    # pooling_r 4 test again
        
        #self.conv = DCN(chi, cho, kernel_size=(3, 3), stride=1, padding=1, dilation=1, deformable_groups=1)
        #self.conv = DCN(chi, cho, kernel_size=3, stride=1, padding=1, dilation=1, deform_groups=1)
        #self.conv = DeformableConv2d(chi, cho, kernel_size=3, padding=1, bias=False)
        
        self.modulator_conv = nn.Conv2d(chi, 9, kernel_size=3, stride=1, padding=1, dilation=1, bias=True)
        #self.modulator_conv = SCConvFDC(chi, 9, stride=1, pooling_r=pooling_r)

        nn.init.constant_(self.modulator_conv.weight, 0.)
        nn.init.constant_(self.modulator_conv.bias, 0.)

        self.regular_conv = nn.Conv2d(chi, cho, kernel_size=3, stride=1, padding=1, dilation=1, bias=False)
    ...
    def forward(self, x):
        ...
        x = torchvision.ops.deform_conv2d(input=x, offset=offset, weight=self.regular_conv.weight,
                                          bias=self.regular_conv.bias, padding=1,
                                          mask=modulator, stride=1, dilation=1)
        
        x = self.actf(x)
        return x
    
class DeformConvSCCA(nn.Module):
    def __init__(self, chi, cho, pooling_r):
        super(DeformConvSCCA, self).__init__()
        self.actf = nn.Sequential(
            nn.BatchNorm2d(cho, momentum=BN_MOMENTUM),
            nn.ReLU(inplace=True)
        )
        
        self.offset_conv = SCConvCA(chi, 18, stride=1, pooling_r=pooling_r)    # pooling_r 4 test again
...
class DLASeg(nn.Module):
    def __init__(self, base_name, heads, pretrained, down_ratio, final_kernel,
        ...
        #self.conv_up_level1 = DeformConv(512, 512)
        ...
        #self.carafe1 = CARAFEPack(channels = 512, scale_factor=2, up_kernel = 5, up_group = 1, encoder_kernel = 3, encoder_dilation = 1, compressed_channels = 64)
        self.conv_cat1 = DeformConv(512, 256)
        #self.conv_sum1 = DeformConv(256, 256)
        #self.conv_cat1 = DeformConv(256, 256)
        #self.conv_cat1 = DeformConvSCCA(512, 256, pooling_r=2)
        
        #self.conv_up_level2 = DeformConv(768, 256)
        ...
        #self.carafe2 = CARAFEPack(channels = 256, scale_factor=2, up_kernel = 5, up_group = 1, encoder_kernel = 3, encoder_dilation = 1, compressed_channels = 64)
        self.conv_cat2 = DeformConv(256, 128)
        #self.conv_sum2 = DeformConv(128, 128)
        #self.conv_cat2 = DeformConv(128, 128)
        #self.conv_cat2 = DeformConvSC(256, 128, pooling_r=2)
        #self.conv_cat2 = DeformConvSC(128, 128, pooling_r=4)
        
        #self.conv_up_level3 = DeformConv(384, 128)
        self.conv_up_level3 = DeformConv(128, 64)
        #self.conv_up_level3 = DeformConv(128, 128)
        #self.conv_up_level3 = DeformConvSC(128, 64, pooling_r=2)
        ...
        #self.carafe3 = CARAFEPack(channels = 128, scale_factor=2, up_kernel = 5, up_group = 1, encoder_kernel = 3, encoder_dilation = 1, compressed_channels = 64)
        #self.conv_cat3 = DeformConv(128, 64)
        #self.conv_sum3 = DeformConv(64, 64)
        #self.conv_cat3 = DeformConv(192, 64)
        #self.conv_cat3 = DeformConv(64, 64)
        #self.conv_cat3_1 = DeformConv(128, 64)
        #self.conv_cat3_2 = DeformConvSC(128, 64, pooling_r=4)
        #self.conv_cat3 = DeformConvSC(64, 64, pooling_r=4)
        self.conv_cat3 = DeformConvSC(128, 64, pooling_r=4)
        ...
        #self.attention_head_1 = CSAModuleV2(in_size=3)
        #self.attention_head_2 = CSAModuleV3(in_size=3)
    ...
    def forward(self, x):
        ...
        #up_level1 = F.interpolate(x[5], scale_factor=2, mode='bilinear', align_corners=True)
        ...
        #up_level1 = self.carafe1(x[5])
        #print("Level1:{}".format(up_level1.shape))

        concat_level1 = self.conv_cat1(torch.cat((up_level1, x[4]), dim=1))
        #concat_level1 = torch.cat((up_level1, x[4]), dim=1)
        #concat_level1 = self.conv_cat1(up_level1 + x[4])
        #sum_level1 = self.conv_sum1(up_level1 + x[4])
        #concat_level1 = concat_level1 + sum_level1
        ...
        #up_level2 = self.carafe2(concat_level1)
        #print("Level2:{}".format(up_level2.shape))

        concat_level2 = self.conv_cat2(torch.cat((up_level2, x[3]), dim=1))
        #concat_level2 = torch.cat((up_level2, x[3]), dim=1)
        #concat_level2 = self.conv_cat2(up_level2 + x[3])
        #sum_level2 = self.conv_sum2(up_level2 + x[3])
        #concat_level2 = concat_level2 + sum_level2
        ...
        #up_level3 = self.carafe3(concat_level2)
        ...
        #sum_level3 = self.conv_sum3(up_level3 + x[2])
        #concat_level3 = concat_level3 + sum_level3
        #concat_level3 = self.conv_cat3(up_level3 + x[2])
        #concat_level3_1 = self.conv_cat3_1(torch.cat((up_level3, x[2]), dim=1))
        #concat_level3 = self.conv_cat3_2(torch.cat((up_level3, x[2]), dim=1))
        #concat_level3 = concat_level3 + concat_level3_1
        ...
        for head in self.heads:
            ...
            #for fpn_idx, fdn_input in enumerate([up_level2, up_level3, concat_level3]):
            for fpn_idx, fdn_input in enumerate([concat_level1, concat_level2, concat_level3]):
                ...
                #if hm_w // fpn_out_w == 2 and head == 'hm':
            ...
            if head == 'hm':
                ...
                #final_out_1 = final_out
                #final_out_1 = self.attention_head_1(final_out_1)
                
                #final_out = self.attention_head_2(final_out)
                #final_out = final_out + final_out_1
                final_out = self.attention_head(final_out)
            ret[head] = final_out
...
def get_pose_net(num_layers, heads, head_conv=256, down_ratio=4):
    model = DLASeg('dlasc{}'.format(num_layers), heads,
                   pretrained=True,
