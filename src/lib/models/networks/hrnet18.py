...
class HighResolutionNet(nn.Module):
    def __init__(self, heads, final_kernel, head_conv):
        ...
        self.stage2, pre_stage_channels = self._make_stage(1, 2, [4, 4], [18, 36], block, 'SUM', num_channels)
        ...
        self.stage3, pre_stage_channels = self._make_stage(4, 3, [4, 4, 4], [18, 36, 72], block, 'SUM', num_channels)
        ...
        self.transition3 = self._make_transition_layer(
            pre_stage_channels, num_channels)
        self.stage4, pre_stage_channels = self._make_stage(3, 4, [4, 4, 4, 4], [18, 36, 72, 144], block, 'SUM', num_channels, multi_scale_output=True)
        ...
        self.ida_up = IDAUp(24, [24, 64, 576], [2 ** i for i in range(3)])
    ...
    def forward(self, x):
        ...
        for i in range(2):
            if self.transition1[i] is not None:
                ...
                x_list.append(x)
        st2_list = self.stage2(x_list)

        x_list = []
        for i in range(3):
            if self.transition2[i] is not None:
                if i < 2:
                    x_list.append(self.transition2[i](st2_list[i]))
                else:
                    x_list.append(self.transition2[i](st2_list[-1]))
            else:
                x_list.append(st2_list[i])
        st3_list = self.stage3(x_list)

        x_list = []
        for i in range(4):
            if self.transition3[i] is not None:
                if i < 3:
                    x_list.append(self.transition3[i](st3_list[i]))
                else:
                    x_list.append(self.transition3[i](st3_list[-1]))
            else:
                x_list.append(st3_list[i])
        st4_list = self.stage4(x_list) #x

        # Upsampling
        #x0_h, x0_w = st4_list[0].size(2), st4_list[0].size(3)
        #x1 = F.interpolate(x[1], size=(x0_h, x0_w), mode='bilinear', align_corners=None)
        #x2 = F.interpolate(x[2], size=(x0_h, x0_w), mode='bilinear', align_corners=None)
        #x3 = F.interpolate(x[3], size=(x0_h, x0_w), mode='bilinear', align_corners=None)

        #final_out = torch.cat([st4_list[0], x1, x2, x3], 1)

        #final_out = self.last_layer(final_out)
        
        out = [st2_list, st3_list, st4_list]
