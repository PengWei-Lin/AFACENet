...
from models.losses import FocalLoss, L1Loss, BinRotLoss ,L1Loss_Balanced
...
class DddLoss(torch.nn.Module):
  def __init__(self, opt):   # original no optimizer
  ...
  def forward(self, outputs, batch):
    ...
    losses = [self.crit(output['hm'], batch['hm']),
              ...
              self.crit_reg(output['dim'], batch['reg_mask'], batch['ind'], batch['dim']),
              self.crit_rot(output['rot'], batch['rot_mask'], batch['ind'], batch['rotbin'], batch['rotres']),
              self.crit_reg(output['wh'], batch['rot_mask'], batch['ind'], batch['wh']), 
              self.crit_reg(output['reg'], batch['rot_mask'], batch['ind'], batch['reg'])]
    
    PCG.pc_backward(losses)
    PCG.step()
    
    hm_loss = hm_loss + losses[0] / opt.num_stacks
    dep_loss = dep_loss + losses[1] / opt.num_stacks
    dim_loss = dim_loss + losses[2] / opt.num_stacks
    rot_loss = rot_loss + losses[3] / opt.num_stacks
    wh_loss = wh_loss + losses[4] / opt.num_stacks
    off_loss = wh_loss + losses[5] / opt.num_stacks
    '''
