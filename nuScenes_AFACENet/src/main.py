  ...
  for epoch in range(start_epoch + 1, opt.num_epochs + 1):
    ...
    if opt.val_intervals > 0 and epoch % opt.val_intervals == 0:
      ...
      for k, v in log_dict_val.items():
        ...
        logger.write('{} {:8f} | '.format(k, v))
      #if log_dict_val[opt.metric] < best:                         # from CenterNet
        #best = log_dict_val[opt.metric]                           # from CneterNet
        #save_model(os.path.join(opt.save_dir, 'model_best.pth'),  # from CenterNet
                   #epoch, model)                                  # from CenterNet
    else:
