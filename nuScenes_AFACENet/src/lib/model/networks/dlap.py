...
def dlap34(pretrained=True, **kwargs):  # DLA-34
...
class DeformConv(nn.Module):
    def __init__(self, chi, cho):
        ...
        #self.conv = DeformConv2d(chi, cho, kernel_size=3, padding=1, bias=False)
        #self.conv = DCN(chi, cho, kernel_size=(3, 3), stride=(1, 1), padding=1, deform_groups=1)
    def forward(self, x):
        x = self.conv(x)
        x = self.actf(x)
        return x
...
class IDAUp(nn.Module):
    ...
    def forward(self, layers, startp, endp):
        for i in range(startp + 1, endp):
            ...
            if i==2: # only for CARAFE
                layers[i] = upsample(layers[i])
            #print("layer[i]{}:{}".format(i,layers[i].shape))
            node = getattr(self, 'node_' + str(i - startp))
            #print("layer[i-1]{}{}".format(i-1,layers[i - 1].shape))
...
class DLASeg(BaseModel):
    def __init__(self, num_layers, heads, head_convs, opt):
        ...
        self.last_level = 5
        self.base = globals()['dlap{}'.format(num_layers)](
        ...
        #self.conv_cat3 = DeformConvSC(128, 64, 4)
    ...
    def img2feats(self, x):
        ...
        #up_level1 = F.interpolate(self.conv_up_level1(x[3]), scale_factor=2, mode='bilinear', align_corners=True)
        #concat_level1 = self.conv_cat1(torch.cat((up_level1, x[2]), dim=1))
        
        #up_level2 = F.interpolate(self.conv_up_level2(concat_level1), scale_factor=2, mode='bilinear', align_corners=True)
        #concat_level2 = self.conv_cat2(torch.cat((up_level2, x[1]), dim=1))
        
        #up_level3 = F.interpolate(self.conv_up_level3(concat_level2), scale_factor=2, mode='bilinear', align_corners=True)
        #concat_level3 = self.conv_cat3(torch.cat((up_level3, x[0]), dim=1))
