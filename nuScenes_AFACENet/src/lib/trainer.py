...
from model.losses import BinRotLoss, WeightedBCELoss, L1Loss_Balanced
  ...
  def forward(self, outputs, batch):
    ...
    for s in range(opt.num_stacks):
      ...
      for head in regression_heads:
        ...
        if head == 'dep' or head == 'amodel_offset':
          losses[head] += self.crit_BL(
            output[head], batch[head + '_mask'],
            batch['ind'], batch[head]) / opt.num_stacks
        elif head in output and (head !='dep' or head != 'amodel_offset'):
          losses[head] += self.crit_reg(
            output[head], batch[head + '_mask'],
            batch['ind'], batch[head]) / opt.num_stacks
